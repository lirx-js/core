...
const $fsh = require('../../misc/fs-helpers.mjs');
...
function resolveImportMetaForFile(entryPath) {
  const destPath = $path.join(DIST_TS_PATH, $path.relative(ROOT_PATH, entryPath));

  return $fsh.createDirectory($path.dirname(destPath)).then(() => {
    if (entryPath.endsWith('.ts')) {
      return $fs.readFile(entryPath, { encoding: 'utf8' }).then((content) => {
...
function resolveImportMeta() {
  return $fsh
    .exploreDirectory(SRC_PATH, (entryPath, entry) => {
      if (entry.isFile()) {
