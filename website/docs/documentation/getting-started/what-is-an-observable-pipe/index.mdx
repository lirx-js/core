...
An essential part of Reactive Programing, is being able to **pipe** our Observable, to build easily **complex dataflow**.

We will start first by defining what is an ObservablePipe.
...
As an example, here's an handmade *ObservablePipe* that emits only distinct received values:
