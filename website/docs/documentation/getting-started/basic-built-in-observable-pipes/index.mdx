...
However, it it a little more complex than the previous one. So don't hesitate to play with it.

### Sharing values of an Observable (only subscribing once)

Last common use case: if we want to subscribe many times to an Observable but subscribe only once to the original one, we may use
...
They are the most powerful tools from this library, however, it requires you to browse carefully the documentation.
