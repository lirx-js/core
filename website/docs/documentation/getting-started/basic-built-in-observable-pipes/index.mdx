...
In this tutorial, we'll **learn** to use some the **most essentials** ObservablePipes provided by `@lirx/core`.

### Filtering and mapping

The most common pipes are [filter$$$](../../../reference/filter-observable-pipe/), [map$$$](../../../reference/map-observable-pipe/),
and [distinct$$$](../../../reference/distinct-observable-pipe/).
...
As you may see this is really similar with the `Array.map` and `Array.filter` functions:
...
Sometimes, we need to delay incoming values and/or discard next values during this period.

The most common pipes are [debounceTime$$$](../../../reference/debounce-time-observable-pipe/)
and [throttleTime$$$](../../../reference/map-observable-pipe/).
...
One of the most used pipe is [switchMap$$$](../../../reference/merge-map-single-observable-pipe/).
...
Last common use case: if we want to subscribe many times to an Observable but subscribe only once to the original once, we may use
[shareRL$$$](../../../reference/share-observable-pipe/share-observable-pipe-with-multicast-replay-last-source/).
...
Plenty of others *ObservablePipes* may be found on the **[reference page](../../../reference/observable-pipe/)**.
