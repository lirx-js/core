...
For example, an HTTP request finishes with a success state (and associated Response's Body)
...
The essentials Notifications are:
...
An voilà ! We've created an Observable from a Promise 🎊.
...
We may also use the function [defaultNotificationObserver](/docs/reference/default-notification-observer/) if we prefer a shorted syntax:
...
**Yes, we could have a shorter code with Promises.**

However, with Observables we may create far more complex data flow - like streaming the response body:
