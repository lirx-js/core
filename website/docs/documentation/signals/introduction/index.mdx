...
In consequence, a part of them (and maybe you, and even me) usually struggle playing with pure *async* pieces of code,
...
This may be fixed using [function$$](/docs/reference/optimized-reactive-function/) but this is another discussion.
...
ends by updating the signal `A` itself. This causes an infinite loop of updates, crashing the application.

Hopefully the framework provides some protections against such cases: it will try to warn you, and throw errors when necessary.
However, it cannot cover all possibilities, but the more the framework is used and tested,
the more it will be improved and will cover such complex cases and prevent such errors.
...
A Signal is similar to a classic variable, however it includes the possibility to subscribe to its changes. Nothing more.

In consequence:

- to get *async* data, you still want to use some Observables. For exemple, using `fromFetch`, `fromAsyncIterable` or `fromEventTarget`.
