...
```ts
interface ISignalMutateFunctionCallback<GValue> {
  (
...
Returns a non-writable Signal (`IReadonlySignal`) which accesses this Signal but does not allow mutation.
...
```ts
const names = signal<string[]>(['Alice']);
