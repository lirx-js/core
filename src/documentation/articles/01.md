...
First, you have to know that, by convention, an Observable ends with `$`, and `@lirx/core` extends this principle
to many functions that generates an Observable with an `$$` at the end.
...
- it is far [faster and smaller](https://github.com/lirx-js/core/blob/main/src/documentation/performances.md) than RxJS,
